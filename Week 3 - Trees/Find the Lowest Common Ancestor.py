...
def lca(root, a, b):
    ...
    that when both nodes are found (for optimization). BFS or DFS traversal could work.
    
    Then, we can start from each node, and work our way up to find the LCA.
    ...
    two lists with at least one element in common. To find the LCA, we could set
    a pointer to the end of the list (representing the root node), and
    repeatedly check if the previous elements in both lists are also the same.
    When they stop being the same, then we've found the LCA. This uses the fact
    that they are both sorted to find the LCA efficiently.
    ...
    their LCA node. This approach is more memory efficient because it does not require
    lists.
        2.1) If the tree cannot be modified, the equivalent data can be stored in a 
        dictionary and the problem can be solved in the same way. The solution below
        implements this. It is still more efficient than solution 1 above, because that
        solution would require yet another 2 data structures (lists of tuples) as well
        as a linear search across them.
    
    '''
    nodes_found = 0
    nodes_info = {root: (None, 0)}
    
    if root == a or root == b:
        nodes_found += 1
    
    # BFS
    # We can store the level along with each node, or we could also only store the nodes
    # and access their level using nodes_info, but that has a worst case TC of O(N).
    q = deque([(root, 0)])
    ...
    while q:
        ...
        if node.left:
            nodes_info[node.left] = (node, lvl+1)
        ...
        if node.right:
            nodes_info[node.right] = (node, lvl+1)
    ...
    # Named tuple or class could make code more readable? 
    # [0] = reference to parent node, [1] = node level
    while p1 != p2:
        if nodes_info[p1][1] > nodes_info[p2][1]:  # compare levels
            p1 = nodes_info[p1][0] # move pointer to parent node
        elif nodes_info[p2][1] > nodes_info[p1][1]:
            p2 = nodes_info[p2][0]
        else:
            p1 = nodes_info[p1][0]
            p2 = nodes_info[p2][0]
